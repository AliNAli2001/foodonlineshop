      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <article className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
            <h2 className="mb-3 text-lg font-semibold text-white">Category Information</h2>
            <p className="text-sm text-slate-200"><span className="text-slate-400">Arabic Name: </span>{category.name_ar}</p>
            <p className="text-sm text-slate-200"><span className="text-slate-400">English Name: </span>{category.name_en}</p>
            <p className="text-sm text-slate-200"><span className="text-slate-400">Featured: </span>{category.featured ? 'Yes' : 'No'}</p>
            {image && <img src={image} alt={category.name_en} className="mt-3 h-48 rounded-xl object-cover" />}
          </article>
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody>
                {rows.length === 0 ? <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-slate-400">No products in this category.</td></tr> : rows.map((p: any) => (
