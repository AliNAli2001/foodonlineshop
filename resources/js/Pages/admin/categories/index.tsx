              ...
              <tbody>
                {rows.length === 0 ? <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-slate-400">{t('admin.pages.categories.index.empty')}</td></tr> : rows.map((cat: any) => (
                  <tr key={cat.id} className="border-t border-white/10">
                    ...
                    <td className="px-4 py-3 text-sm">{cat.featured ? <span className="rounded-full bg-emerald-400/20 px-2 py-0.5 text-xs dark:text-emerald-200 ring-1 ring-emerald-300/30">{t('common.yes')}</span> : <span className="rounded-full bg-slate-300/20 px-2 py-0.5 text-xs text-slate-200 ring-1 ring-slate-300/30">{t('common.no')}</span>}</td>
                    <td className="px-4 py-3"><div className="flex flex-wrap gap-2"><Link href={`/admin/categories/${cat.id}`} className="rounded-lg border border-cyan-400/40 bg-cyan-50 px-2.5 py-1 text-xs text-cyan-700 hover:bg-cyan-100 dark:border-cyan-300/30 dark:bg-cyan-400/10 dark:text-cyan-200 dark:hover:bg-cyan-400/20">{t('common.view')}</Link><Link href={`/admin/categories/${cat.id}/edit`} className="rounded-lg border border-amber-400/40 bg-amber-50 px-2.5 py-1 text-xs text-amber-700 hover:bg-amber-100 dark:border-amber-300/30 dark:bg-amber-400/10 dark:text-amber-200 dark:hover:bg-amber-400/20">{t('common.edit')}</Link><button onClick={() => removeCategory(cat.id)} className="rounded-lg border border-rose-400/40 bg-rose-50 px-2.5 py-1 text-xs text-rose-700 hover:bg-rose-100 dark:border-rose-300/30 dark:bg-rose-500/10 dark:text-rose-200 dark:hover:bg-rose-500/20">{t('common.delete')}</button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
