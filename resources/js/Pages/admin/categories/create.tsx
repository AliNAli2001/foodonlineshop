...
export default function CategoriesCreate() {
  ...
  const { data, setData, post, processing, errors } = useForm({
    entries: [{ name_ar: '', name_en: '', featured: false }],
  });
  ...
  const addRow = () => {
    setData('entries', [...data.entries, { name_ar: '', name_en: '', featured: false }]);
  };
  ...
  const updateEntry = (index: number, key: 'name_ar' | 'name_en' | 'featured', value: string | boolean) => {
    setData(
      'entries',
  ...
  return (
    <AdminLayout title={t('admin.pages.categories.create.title')}>
      <div className="mx-auto max-w-3xl space-y-6">
        <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5"><h1 className="text-2xl font-bold text-white">{t('admin.pages.categories.create.heading')}</h1></section>
          ...
          {data.entries.map((entry, index) => (
            <div key={index} className="rounded-xl border border-white/10 bg-white/[0.03] p-3 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-slate-200">{t('admin.pages.categories.create.heading')} #{index + 1}</p>
              ...
              <Field label={t('admin.pages.categories.form.arabicName')} error={(errors as any)[`entries.${index}.name_ar`]}><input value={entry.name_ar} onChange={(e) => updateEntry(index, 'name_ar', e.target.value)} className={inputClass} required /></Field>
              <Field label={t('admin.pages.categories.form.englishName')} error={(errors as any)[`entries.${index}.name_en`]}><input value={entry.name_en} onChange={(e) => updateEntry(index, 'name_en', e.target.value)} className={inputClass} required /></Field>
              <label className="flex items-center gap-2 text-sm text-slate-200"><input type="checkbox" checked={entry.featured} onChange={(e) => updateEntry(index, 'featured', e.target.checked)} /> {t('admin.pages.categories.form.featured')}</label>
            </div>
          ))}
