      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          <div>
            ...
            <p className="text-sm text-slate-300">{t('admin.pages.inventory.product.totalStock')}: {product?.stock_available_quantity ?? 0}</p>
          </div>
          <div className="flex gap-2">
            <Link href={`/admin/inventory/${product.id}/create`} className="rounded-xl bg-amber-400 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-amber-300">{t('admin.pages.inventory.product.addBatch')}</Link>
        ...
        <section className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
          <h2 className="border-b border-white/10 px-4 py-3 text-lg font-semibold text-white">{t('admin.pages.inventory.product.batches')}</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead className="bg-white/[0.03]"><tr>{[
                t('admin.pages.inventory.form.batchNumber'),
                t('admin.pages.inventory.form.expiryDate'),
                t('admin.pages.inventory.form.costPrice'),
                t('admin.pages.inventory.index.stock'),
                t('common.status'),
                t('common.actions'),
              ].map((h) => <th key={h} className="px-4 py-3 text-left text-xs uppercase tracking-[0.12em] text-slate-400">{h}</th>)}</tr></thead>
              <tbody>
                {batchRows.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-slate-400">{t('admin.pages.inventory.product.emptyBatches')}</td></tr>
                ) : (
                  batchRows.map((b: any) => (
                    <tr key={b.id} className="border-t border-white/10">
                      ...
                      <td className="px-4 py-3"><div className="flex gap-2"><Link href={`/admin/inventory/${b.id}`} className="rounded-lg  border border-cyan-300/40 bg-cyan-50 px-2.5 py-1 text-xs text-cyan-700 hover:bg-cyan-200 dark:border-cyan-400/30 dark:bg-cyan-400/10 dark:text-cyan-200 dark:hover:bg-cyan-400/20">{t('common.view')}</Link><Link href={`/admin/inventory/${b.id}/edit`} className="rounded-lg border border-amber-300/40 bg-amber-50 px-2.5 py-1 text-xs text-amber-700 hover:bg-amber-100  dark:border-amber-400/30 dark:bg-amber-400/10 dark:text-amber-200 dark:hover:bg-amber-400/20">{t('common.edit')}</Link></div></td>
                    </tr>
                  ))
                )}
              </tbody>
