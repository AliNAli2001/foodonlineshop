...
export default function TagsCreate() {
  const { t } = useI18n();
  const { data, setData, post, processing, errors } = useForm({ entries: [{ name_ar: '', name_en: '' }] });
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          {data.entries.map((entry, index) => (
            <div key={index} className="rounded-xl border border-white/10 bg-white/[0.03] p-3 space-y-3">
              ...
              <Field label={t('admin.pages.tags.form.arabicName')} error={(errors as any)[`entries.${index}.name_ar`]}><input value={entry.name_ar} onChange={(e) => updateEntry(index, 'name_ar', e.target.value)} className={inputClass} required /></Field>
