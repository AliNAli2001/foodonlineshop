    ...
    <AdminLayout title={t('admin.pages.tags.show.title')}>
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3"><h2 className="text-lg font-semibold text-white">{t('admin.pages.tags.show.products')} ({products?.total || rows.length})</h2><Link href="/admin/products/create" className="rounded-lg border border-cyan-300/30 bg-cyan-400/10 px-3 py-1.5 text-xs text-cyan-200 hover:bg-cyan-400/20">{t('admin.pages.tags.show.addProduct')}</Link></div>
          <div className="overflow-x-auto"><table className="min-w-full"><thead className="bg-white/[0.03]"><tr>{[
            ...
            t('common.actions'),
          ].map((h) => <th key={h} className="px-4 py-3 text-left text-xs uppercase tracking-[0.12em] text-slate-400">{h}</th>)}</tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-slate-400">{t('admin.pages.tags.show.emptyProducts')}</td></tr> : rows.map((p: any) => <tr key={p.id} className="border-t border-white/10"><td className="px-4 py-3 text-sm text-slate-200">{p.name_en || p.name_ar}</td><td className="px-4 py-3 text-sm text-slate-200">${Number(p.selling_price ?? p.price ?? 0).toFixed(2)}</td><td className="px-4 py-3 text-sm text-slate-200">{p.stock_available_quantity ?? p.inventory?.stock_quantity ?? 0}</td><td className="px-4 py-3 text-sm text-slate-200">{p.featured ? t('common.yes') : t('common.no')}</td><td className="px-4 py-3"><div className="flex gap-2"><Link href={`/admin/products/${p.id}`} className="rounded-lg border border-cyan-300/30 bg-cyan-400/10 px-2.5 py-1 text-xs text-cyan-200 hover:bg-cyan-400/20">{t('common.view')}</Link><Link href={`/admin/products/${p.id}/edit`} className="rounded-lg border border-amber-300/30 bg-amber-400/10 px-2.5 py-1 text-xs text-amber-200 hover:bg-amber-400/20">{t('common.edit')}</Link></div></td></tr>)}</tbody></table></div>
