    ...
    <AdminLayout title="Company Details">
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          <div><h1 className="text-2xl font-bold text-white">{company.name_en}</h1><p className="text-sm text-slate-300">Company profile and linked products.</p></div>
          <div className="flex gap-2"><Link href={`/admin/companies/${company.id}/edit`} className="rounded-xl bg-amber-400 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-amber-300">Edit</Link><Link href="/admin/companies" className="rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm text-slate-200 hover:bg-white/10">Back</Link></div>
        </section>

        <section className="grid gap-6 lg:grid-cols-2">
          <article className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
            <h2 className="mb-3 text-lg font-semibold text-white">Company Information</h2>
            <p className="text-sm text-slate-200"><span className="text-slate-400">Arabic Name: </span>{company.name_ar}</p>
            <p className="text-sm text-slate-200"><span className="text-slate-400">English Name: </span>{company.name_en}</p>
        ...
        <section className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead className="bg-white/[0.03]"><tr>{['Name', 'Price', 'Stock', 'Featured', 'Actions'].map((h) => <th key={h} className="px-4 py-3 text-left text-xs uppercase tracking-[0.12em] text-slate-400">{h}</th>)}</tr></thead>
              <tbody>
                {rows.length === 0 ? <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-slate-400">No products for this company.</td></tr> : rows.map((p: any) => (
